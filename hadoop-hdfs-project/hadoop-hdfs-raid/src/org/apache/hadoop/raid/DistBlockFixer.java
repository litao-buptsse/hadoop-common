...
import org.apache.hadoop.raid.RaidDFSUtil;
...
public class DistBlockFixer extends BlockFixer {
  ...
  class JobMonitorThread extends Thread {

	Job job;
	DistBlockFixer fixer;
	
	public JobMonitorThread(Job job, DistBlockFixer fixer) {
	   	this.job = job;
	   	this.fixer = fixer;
	}
  ...
  private void checkJob(Job job) throws IOException {
	 Set<Job> jobs = jobIndex.keySet();
	 for(Job j : jobs) {
		 if(j.getJobID().getId() == job.getJobID().getId()) {
   ...
   */
  private void startJob(String jobName, List<Path> corruptFiles) 
    throws IOException, InterruptedException, ClassNotFoundException {
    ...
    jobConf.setInt("mapreduce.task.timeout", 0);
    ...
    service.submit(new JobMonitorThread(job, this));
  ...
  private List<Path> getCorruptFiles() throws IOException {
	...
	List<String> all_files = new LinkedList<String>(); 
	  
	FileSystem fs = FileSystem.get(new Configuration());
	if(fs instanceof ViewFileSystem) {
	  for(FileSystem fss : fs.getChildFileSystems()) {
		String[] files = RaidDFSUtil.getCorruptFiles((DistributedFileSystem)fss);
		for(String fi : files) {
		  all_files.add(fi);
		}
	  }
	} else if(fs instanceof DistributedFileSystem) {
		String[] files = RaidDFSUtil.getCorruptFiles((DistributedFileSystem)fs);
		for(String fi : files) {
		  all_files.add(fi);
		} 	
	} else {
	  throw new IOException("can not fond correct filesystem.");	
	}
	
    /*DistributedFileSystem dfs = (DistributedFileSystem) 
      FED_HAUtil.getDistributedFileSystem((new Path("/")).getFileSystem(getConf()));*/
  ...
  protected static void loginAsNameNodeKeyTab(Configuration conf) throws IOException {
    if (UserGroupInformation.isSecurityEnabled()) {
      ...
      raidBlockfixerUserName = SecurityUtil.getServerPrincipal(raidBlockfixerUserName, (String)null);

...
public abstract class RaidNode implements RaidProtocol {
      ...
      while (running) {
        ...
        for (PolicyInfo info: allPolicies) {
          ...
          if (!shouldSelectFiles(info)) {
        	System.out.println("xxx should not select any file to raid.");
            continue;
          }
        ...
        System.out.println("xxx raid files size:" + filteredPaths.size());
        for(FileStatus fStatus : filteredPaths) {
        	System.out.println("xxx raid files name:" + fStatus.getPath().toString());	
        }
          } catch (Exception e) {
  ...
  static private void generateParityFile(Configuration conf, FileStatus stat,
    ...
    inFs.setTimes(inpath, stat.getModificationTime(), -1L);
